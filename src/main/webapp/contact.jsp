...
String urlSuccess = "/contact.jsp?CMCCode=" + CMCCode + "&amp;extLang=" + lang;
...
int rowCount = contact_jsp_cmrow.getCMRow(CMCCode,"CCCRRank DESC, CMRDateCreated DESC").getRetInt();
...
<fieldset id="field1">
  ...
  <label for="copyReq__hidden" style="width:auto;"><%=lb.get("copyReq" + lang)%>&nbsp;</label><input id="copyReq__hidden" name="copyReq__hidden" type="checkbox" value="1" class="checkbox" <%if ("1".equals(copyReq__hidden)) out.print("checked");%>/><br/>
  ...
  <button type="button" class="btn btn-primary btn-md" onclick="if (validateForm(document.getElementById('mailForm')) == true) document.getElementById('mailForm').submit();" name="submitbtn" id="submitbtn"><span class="glyphicon glyphicon-envelope"></span>&nbsp;<%=lb.get("sendButton" + lang)%></button>
...
<%if (rowCount > 0) {%><div id="contactFormAddres"><%=contact_jsp_cmrow.getColumn("CMRText" + lang)%></div><%}%>
