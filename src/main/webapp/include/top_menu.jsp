      ...
      <li style=""><a href="<%="http://" + serverName + "/" + (defLang.equals(lang) ? "" : "?extLang=") + lang%>"><span style="display: block;" class="glyphicon glyphicon-home"></span></a></li>
			...
			<li><a href="#_" class="megamenu_drop"><%=top_jsp_lb.get("prdTitle" + lang)%></a><!-- Begin Item -->
				<div class="dropdown_fullwidth megaproduct"><!-- Begin Item Container -->
					...
					<div id="megamenuAllCategories" class="col_3">
						<%if (MEGAMENU_PRDPROMO_URL != null && MEGAMENU_PRDPROMO_URL.length() > 0){%><a href="<%=MEGAMENU_PRDPROMO_URL%>"><%}%><img src="/images/products_mega_menu_promo<%=lang%>.png" alt="logo" style="width:100%;"/><%if (MEGAMENU_PRDPROMO_URL != null && MEGAMENU_PRDPROMO_URL.length() > 0){%></a><%}%>
			...
			<%if ("<a>".equals(menuOption.getTag()) && menuOption.getCode().length() == 4 && "1".equals(menuOption.getParent())) {%>
				<li>
					...
					<div class="dropdown_5columns dropdown_container"><%--το class dropdown_5columns ορίζει το width--%><!-- Begin Item Container -->
					<ul class="dropdown_flyout">
			<%
				menuLevel = 1;
			}%>
